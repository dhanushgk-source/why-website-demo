...
const whyItems = comparisonRows.map((label) => ({ label, value: '✓', isCheck: true }))
...
const othersItems = [
  ...
  { label: 'Background check', value: 'Sometime', isCheck: null },
...
export default function WhyDifferent() {
  ...
  const activeTabClass = 'bg-gradient-to-r from-[#2F4A7D] to-[#2DD4BF] text-white shadow-lg'
  const inactiveTabClass = 'text-gray-600'

  const currentItems = activeTab === 'why' ? whyItems : othersItems

  return (
    <section ref={sectionRef} id="why-section" className="py-24 px-6 text-center">
      ...
      <p className="fade-inner text-gray-500 text-xl mb-16">Not another marketplace</p>

      {/* Toggle */}
      <div className="fade-inner inline-flex bg-gray-200 p-2 rounded-full shadow-inner mb-20">
        <button
          onClick={() => setActiveTab('why')}
          className={`tab px-10 py-3 rounded-full font-semibold text-lg transition-all duration-300 ${activeTab === 'why' ? activeTabClass : inactiveTabClass}`}
        >
          WHY
        </button>
        <button
          onClick={() => setActiveTab('others')}
          className={`tab px-10 py-3 rounded-full font-semibold text-lg transition-all duration-300 ${activeTab === 'others' ? activeTabClass : inactiveTabClass}`}
        >
          Others
        </button>
      </div>
        ...
        <div className="space-y-8">
          {currentItems.map((item) => (
            ...
            >
              ...
              {item.isCheck === null && (
                <div className="px-4 py-2 rounded-full bg-gray-200 border border-gray-300 text-gray-600 text-sm font-semibold shadow-inner whitespace-nowrap">
                  {item.value}
                </div>
              )}
            </div>
