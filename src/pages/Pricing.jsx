    ...
    async function loadData() {
      try {
        ...
        if (fetchedPlans && fetchedPlans.length > 0) {
          // Map backend plans to rich structure if available
          const mapped = fetchedPlans.map((p, i) => ({
            icon: i === 0 ? Building2 : Car,
            title: p.title,
            desc: p.description || (i === 0 ? 'Professional support inside hospitals and clinics.' : 'Companionship and support for your travel journey.'),
            dayPrice: p.price ? p.price.replace(/[^0-9,]/g, "") : (i === 0 ? '1,499' : '999'),
            nightPrice: p.price_night ? p.price_night.replace(/[^0-9,]/g, "") : (i === 0 ? '2,499' : '1,499'),
            includedHours: p.billing_cycle ? `Per ${p.billing_cycle}` : 'Up to 4 Hours',
            includes: Array.isArray(p.features) && p.features.length > 0 ? p.features : DEFAULT_PLANS[i % 2].includes,
            extraNote: 'Extra hours will be charged additionally beyond the included 4 hours.',
